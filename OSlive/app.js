...
angular.module("oslive", ['ngAnimate'])
	...
	.controller('homeController', function ($scope) {
		...
		$scope.escalonadorFila1 = "info"
		$scope.infoFilas = "infoFilas"
			...
			if (!$scope.processo.edit) {
				...
				$scope.processos.forEach(function (process, i) {
					if (process.nome === $scope.processo.nome) {
						...
						process.fila = $scope.processo.fila;
					}
				});
				$scope.processo = null;
			}
		...
		$scope.simular = function () {
			...
			var algoritmoMF1 = $scope.escalonadorFila1;

			var algoritmo1 = $scope.escalonadorFila1;
			var algoritmo2 = $scope.escalonadorFila2;
			...
			if (algoritmo != null && algoritmo != "info") {
				if (lista.length != 0) {
					...
					if (algoritmo == "MF") {
						$scope.resultados = simulandoMultFilas(quantum1);
						if(algoritmo1 == "FIFO" && algoritmo2 == "RR"){
							$scope.resultados2 = pc1.tabelaResultado();
							}else{
								$scope.resultados2 = pc.tabelaResultado();
							}
						drawChart($scope.resultados2);
						$.notify("Escalonamento Multiplas Filas!", "success");
					}
		...
		function simulandoMultFilas(q) {

			var escalonar = new PrioridadePreemp();
			var escalonar1 = new FIFO();
			var escalonar2 = new FIFO();
			...
			if(qtdFilas == 2)
			{
				if (algoritmo1 == "FIFO" && algoritmo2 == "FIFO") {
				...
				while (!pc.vazio() || !escalonar.vazio() || cpu.ocupado) {
					...
					if (!escalonar.vazio()) {
						...
						else if (escalonar.topo().fila < cpu.processo.fila) {
					...
					if (aux != null)
						resultado.push({ nome: aux.nome, cor: aux.cor, fila: aux.fila });
					else
						resultado.push({ nome: "-", cor: "#FFFFFF", fila:"-" });
				...
				for (var i = 0; i < resultado.length; i++) {
					status.push({ tempo: i, nome: resultado[i].nome, cor: resultado[i].cor, fila: resultado[i].fila });
				}
				return status;
				
				}else if(algoritmo1 == "FIFO" && algoritmo2 == "RR"){

					while (!pc.vazio() || !escalonar.vazio() || cpu.ocupado) {
						aux = pc.processosPorTempo(time);
						var x = 0;

						while (aux.length > 0 && aux[x].fila == "1") {
							escalonar.addProcesso(aux.shift());
							x++;
						}
						...
						if (aux != null && aux.fila == "1")
							resultado.push({ nome: aux.nome, cor: aux.cor , fila: aux.fila, v: true});
						else
							resultado.push({ nome: "-", cor: "#FFFFFF" , fila: "-", v: false});
					...
					for (var i = 0; i < resultado.length; i++) {
						if(resultado[i].nome == "-")
						{
							status.push({ tempo: i,nome: resultado[i].nome, cor: resultado[i].cor, fila:resultado[i].fila, v: false});
							
						}
							
						else
						{
							status.push({ tempo: i,nome: resultado[i].nome, cor: resultado[i].cor, fila:resultado[i].fila , v: true});
							
						}
							
					}

					while (!pc1.vazio() || !escalonar1.vazio() || cpu1.ocupado) {
						aux1 = pc1.processosPorTempo(time1);
						var j = 0;
						
						while (aux1.length > 0 && aux1[x].fila == "2"){
								escalonar1.addProcesso(aux1.shift());
								j++;
						}
							
		
						if (chaveamento > 0) {
							aux1 = cpu1.retiraProcesso();
							escalonar1.addProcesso(aux1);
							chaveamento = 0;
						}
		
						if (tmp == 0 && !escalonar1.vazio())
							cpu1.alocaProcesso(escalonar1.escolherProcesso());
		
						if (cpu1.ocupado) {
							tmp++;
							aux1 = cpu1.act();
							if (!cpu1.ocupado) {
								pc1.addfinalizado(aux1);
								tmp = 0;
							}
						}
						else
						...
						if (aux1 != null && aux1.fila == "2")
							resultado1.push({ nome: aux1.nome, cor: aux1.cor,fila: aux1.fila , v: true});
						else
							resultado1.push({ nome: "-", cor: "#FFFFFF", fila: "-", v: false });
					...
					for (var i = 0; i < resultado1.length; i++) {	
							if(resultado1[i].nome == "-")
								status1.push({ tempo: i,nome: resultado1[i].nome, cor: resultado1[i].cor, fila:resultado1[i].fila, v: false});	
							else
								status1.push({ tempo: i,nome: resultado1[i].nome, cor: resultado1[i].cor, fila:resultado1[i].fila , v: true});
					}

			
				status2 = status.concat(status1);
				...
				for(var i = 0; i < status2.length; i++)
				{
					if(resultado[i] != undefined ){
						if(resultado[i].v == true || resultado[i].v == false && resultado1[i] == null)
							status3.push({ tempo: i,nome: resultado[i].nome, cor: resultado[i].cor, fila:resultado[i].fila });
								if(resultado1[i] != null)
									{
									if(resultado[i].v == true && resultado1[i].v == true)
								{
										resultado1.splice(i, 0,({nome : "-", cor: "#FFFFFF", fila: "-", v: false }) )
										var j = i + 1;
										while(resultado1[j] != null)
										{
											resultado1[j].tempo = j;
					...
					if(resultado[i] == undefined)
						{
							resultado[i] = ({ nome: "-", cor: "#FFFFFF", fila: "-", v : false });
						}

					if(resultado1[i] != undefined && resultado[i].v == false){
						
						if(resultado1[i].v == true || resultado1[i].v == false && resultado[i].v == false);
							status3.push({ tempo: i,nome: resultado1[i].nome, cor: resultado1[i].cor, fila:resultado1[i].fila });
					}
		...
		function simulandoPrioPremp() {
			...
			for (var i = 0; i < resultado.length; i++) {
				status1.push({ tempo: i, nome: resultado[i].nome, cor: resultado[i].cor });
			}
			return status1;
		}
		...
		function validar() {		//verifica se o primeiro processo inicializa com zero
			...
			for (var i = 0; i < $scope.processos.length; i++) {
				if ($scope.processos[0].chegada != 0 || $scope.processos[0].chegada == null) {
					...
					var cor = gera_cor();
					pro.push(new Processo($scope.processos[i].nome, $scope.processos[i].chegada, $scope.processos[i].execucao, $scope.processos[i].prioridade, $scope.processos[i].cor, $scope.processos[i].fila));
				}
			}
			return pro;
		}
